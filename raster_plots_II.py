...
def freq_cleaner(data, threshold):
    ...
    # input:
    # data: matrix/array, which shows the arrays of frequencies and amplitudes for each day
    # output:
    ...
    for i in range(len(data)):
        ...
        for j in reversed(range(len(current_freqs))):
            print('current length: ', len(current_freqs))
            print('j: ', j)
            cf = current_freqs[j]
            ...
            for k in range(j + 1, len(current_freqs)):
                print('k: ', k)
                # comparing_freqs = data[k]
                comparing_freq = current_freqs[k]
                break_k = False
                print(cf, ' is compared to  ',  comparing_freq)
                if np.abs(cf - comparing_freq) < threshold:
                    if comparing_freq < cf:
                        # print('DELETE: ', data[k][l], data[k].shape, l)
                        data[k] = np.delete(data[i], k, axis=0)
                    else:
